...
- [ ] Task: Write Failing Component/Unit Tests for SettingsModal & AgentConfigurator (TDD Red Phase)
  - [ ] Write unit test verifying `AgentConfigurator` behavior when `settings` prop is missing or partial
  - [ ] Write unit test verifying `SettingsModal` successfully renders `Swarm Agents` tab
  - [ ] Run test suite via `vitest` and confirm Red phase test failure
